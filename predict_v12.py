#!/usr/bin/env python3
"""
predict_v12.py — NBA Player Props Model Prediction Engine
...
v12 changes vs v11:
  - Version strings synchronized to 2026-03-09-v12
  - Removed local ADV_FIELDS definition — now imports ALL_ADV_FIELDS
    from feature_engineering so training and inference use identical fields
  - Singles written to disk BEFORE SGP generation (timeout safety preserved)
...
def filter_sgp_candidates(singles: list) -> list:
    ...
    Without this gate, large single counts make SGP generation unmanageable.
    Strategy: keep top SGP_MAX_PER_GAME picks per game by EV, then hard-cap
    at SGP_ABSOLUTE_CAP total.

    Combo stats (pra, pr, pa, ra, stocks, tov) are excluded from SGPs —
    their within-game correlations are trivially high and produce misleading
...
def print_singles_summary(picks: list):
    print(f"\n{'='*70}")
    print(f"  NBA Player Props Model — SINGLES ({len(picks)} picks above {MIN_EV:.1%} EV)")
    print(f"{'='*70}")
    for p in picks[:25]:
...
def print_sgp_summary(sgps: list):
    print(f"\n{'='*70}")
    print(f"  NBA Player Props Model — SGPs ({len(sgps)} candidates above {MIN_EV:.1%} EV)")
    print(f"{'='*70}")
    for s in sgps[:10]:
...
def main():
    logger.info("=" * 60)
    logger.info("NBA Player Props Model PREDICTIONS — VERSION 2026-03-09-v12")
    logger.info("=" * 60)
    ...
    if not models:
        sys.exit("No models in model_cache/. Run train_v12.py first.")

    stats_path = DATA_DIR / "player_game_stats.parquet"
    adv_path   = DATA_DIR / "advanced_stats.parquet"
    if not stats_path.exists():
        sys.exit("No stats data. Run train_v12.py first.")
