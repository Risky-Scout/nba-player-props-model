#!/usr/bin/env python3
"""
grade_darko_v4.py  VERSION: 2026-02-28-v1
===========================================
Syndicate-grade grading: CLV tracking, ROI, calibration, drawdown analysis.

Usage:
    python3 grade_darko_v4.py                   # grade yesterday
    python3 grade_darko_v4.py --date 2026-02-27 # grade specific date
    python3 grade_darko_v4.py --report           # cumulative performance report

Outputs:
    graded/graded_{date}.csv        Row-level graded predictions
    graded/performance_log.csv      Cumulative rolling log (appended)
    graded/cumulative_report.json   Aggregated metrics (rewritten each run)
"""
...
def compute_profit(result: str, bet_odds: int) -> float:
    """
    Flat 1-unit bet.
    HIT  → profit  = decimal - 1  (e.g. +0.909 at -110)
    MISS → profit  = -1.0
    PUSH → profit  = 0.0
    """
    if result == "HIT":
...
# ── Grade individual predictions ──────────────────────────────────────────────

def grade_single(pred: dict, actual_stat: float | None) -> dict:
    """
    Grade one prediction row.
    Returns augmented dict with: result, profit, actual, clv_proxy.
    """
    p = dict(pred)

    if actual_stat is None:
        p["result"]     = "NO_ACTION"  # player did not play / stat unavailable
        p["profit"]     = 0.0
        p["actual"]     = None
        p["clv_proxy"]  = None
        return p
    ...
    # CLV proxy = model_prob(over) - market_implied_prob(over) at time of bet
    # Positive CLV means we had an edge vs the market at bet time
    if side == "OVER":
...
# ── Fetch actual stats from BDL ───────────────────────────────────────────────

def fetch_actual_stats(target_date: str) -> dict[tuple, float]:
    """
    {(player_id, stat_key): actual_value} for a given date.
    Returns empty dict on failure (do not crash the grader).
    """
    try:
...
def grade_date(target_date: str) -> pd.DataFrame:
    ...
    for pred in preds:
        ...
        stat = pred.get("stat", "")
        actual_val = actuals.get((pid, stat))  # None if not available
        graded.append(grade_single(pred, actual_val))

    df = pd.DataFrame(graded)

    # Filter out NO_ACTION rows for stats calculations but keep in CSV
...
def compute_metrics(df: pd.DataFrame, label: str = "") -> dict:
    """
    Compute key metrics from a graded DataFrame.
    Only uses HIT/MISS/PUSH rows (excludes NO_ACTION).
    """
    ...
    # Sharpe (assuming unit bets, benchmark = 0)
...
def print_report(df: pd.DataFrame, date_label: str = ""):
    """Print syndicate-style grading report."""
    ...
    if len(clv_vals) > 0:
        print(f"\n  {'─'*68}")
        print(f"  CLV DISTRIBUTION (model_prob − market_implied at bet time):")
...
# ── Cumulative performance log ─────────────────────────────────────────────────

def update_performance_log(df: pd.DataFrame, target_date: str):
    """Append today's graded rows to the cumulative performance log."""
    active = df[df["result"].isin(["HIT", "MISS", "PUSH"])].copy()
    if active.empty:
        return
    ...
    if PERF_LOG.exists():
        ...
        # Deduplicate: drop any rows already graded for this date
...
def _rebuild_cumulative_report(log: pd.DataFrame):
    """Recompute and save cumulative stats from full log."""
...
def print_cumulative_report():
    """Print the saved cumulative performance report."""
    ...
    print("\nALL TIME:")
    show_metrics(report.get("overall", {}))

    for wkey in ["rolling_30", "rolling_60"]:
        r = report.get(wkey, {})
        if r.get("n_bets", 0) > 0:
            print(f"\n{wkey.upper().replace('_',' ')} BETS:")
            show_metrics(r)
    ...
    # Calibration from full log
    if "model_prob" in bet.columns:
        print(f"\n{'─'*72}")
        print("CALIBRATION (all-time, by model_prob bucket):")
        print(f"  {'Bucket':12s} {'N':>5}  {'Pred':>7}  {'Actual':>7}  {'Error':>7}")
        for lo, hi in [(0.40,0.50),(0.50,0.55),(0.55,0.60),(0.60,0.65),(0.65,0.70),(0.70,1.0)]:
            mask = (bet["model_prob"] >= lo) & (bet["model_prob"] < hi)
...
def main():
    ...
    parser.add_argument("--report", action="store_true",
                        help="Print cumulative performance report only")
