...
| as_of_date selection? | **Phase 13G: strict previous-day-in-America/New_York by default.** `scripts/resolve_previous_day_et_target.py` computes `target_date_et = (now_ET - 1 day).date()` and verifies completeness in `data/player_game_stats.parquet` (≥ 50 player-game rows AND `max_outcome_date_in_file >= target_date_et`). If yesterday-in-ET is incomplete, the resolver exits non-zero and the workflow halts cleanly with `halted_reason=previous_day_data_not_ready`. Operators can override via workflow_dispatch input `as_of_date` (manual date), or set `allow_stale_safe_date=true` to fall back to the most recent complete date with `stale_fallback_used=true` recorded. The legacy Phase 13F resolver remains available for backwards compat behind the orchestrator's `--use-legacy-resolver` flag. |
| 4 PASS lines on success | `TRAINING_AUTOMATION_VERIFICATION_PASS`, `TRAINING_AUTOMATION_REAL_TRAINING_VERIFICATION_PASS`, `PREVIOUS_DAY_NO_LEAKAGE_VERIFICATION_PASS`, `DEREK_WOO_CHAMPION_DEPENDENCY_PASS`, plus `DAILY_AUTOMATION_HEALTH_PASS`. The verifier chains `scripts/verify_previous_day_no_leakage.py` and `scripts/verify_derek_woo_champion_dependency.py` automatically when the real-training path passes. |
