# PMF Variance Experience Study — May 30, 2026

_Actual-to-expected (A/E) review of settled PMF predictions, as-of `2026-05-30` over a 60-day lookback._

## Executive summary

- **1,825** settled rows from **2026-04-17** through **2026-05-28** (28 delivery dates with at least one settled row).
- **Mean A/E = 1.103** — actual outcomes ran +10.3% relative to expected means in this sample.
- **Variance A/E = 0.840** — PMF spread is reasonably close overall (the well-calibrated band is 0.80–1.20), slightly wide overall.
- **Standardized residual: mean = 0.144, sd = 0.993** — slight positive bias and dispersion close to calibrated (target sd = 1.00).
...
|---|---:|
| rows | 1,825 |
| actual_mean (per row) | 6.118 |
| expected_mean (per row) | 5.546 |
| **mean_AE** | **1.1033** |
| Σ squared residual | 25324.43 |
| Σ expected variance | 30135.84 |
| **variance_AE** | **0.8403** |
| standardized_residual_mean | 0.1440 |
| standardized_residual_sd | 0.9930 |
| pmf_nll_mean | 2.5119 |
| pmf_rps_mean | 0.1127 |
| model_brier (over/under) | 0.2690 |
| market_brier (over/under) | 0.2466 |
| model_logloss (over/under) | 0.7405 |
| market_logloss (over/under) | 0.6885 |
| coverage @ 10 / 25 / 50 / 75 / 90 | 0.203 / 0.300 / 0.501 / 0.761 / 0.910 |
...
|---|---|---:|---:|---:|---:|---:|
| role_bucket | `core` | 65 | **1.432** | 1.164 | 1.115 | 2.734 |
...
| Dimension | Bucket | n | variance_AE | std_resid_sd | mean_AE | NLL |
|---|---|---:|---:|---:|---:|---:|
| edge_bucket | `0_to_5pct` | 62 | **0.532** | 0.943 | 0.854 | 2.646 |
| injury_context | `unknown` | 37 | **0.576** | 0.776 | 1.133 | 2.143 |
| line_bucket | `1_to_1p5` | 269 | **0.654** | 0.986 | 1.096 | 1.925 |
| line_bucket | `20_to_25` | 47 | **0.668** | 1.009 | 1.082 | 4.416 |
| line_bucket | `2_to_2p5` | 75 | **0.719** | 1.225 | 1.070 | 2.388 |
| line_bucket | `5_to_8` | 88 | **0.690** | 0.878 | 1.073 | 2.497 |
| line_bucket | `ge_10` | 34 | **0.669** | 0.823 | 1.106 | 2.698 |
| line_bucket | `ge_25` | 63 | **0.601** | 0.803 | 1.045 | 3.660 |
| line_bucket | `le_half` | 326 | **0.429** | 0.687 | 1.015 | 1.256 |
| low_line_discrete | `yes` | 595 | **0.549** | 0.836 | 1.064 | 1.559 |
| p0_bucket | `20_to_50pct` | 318 | **0.670** | 0.934 | 1.040 | 1.840 |
| p0_bucket | `ge_50pct` | 291 | **0.461** | 0.677 | 1.267 | 1.261 |
...
| role_bucket | `starter` | 275 | **0.642** | 0.877 | 1.025 | 2.489 |
| stat | `blk` | 224 | **0.679** | 0.750 | 1.287 | 1.545 |
| stat | `fg3m` | 219 | **0.687** | 1.111 | 0.965 | 2.080 |
| stat | `stl` | 241 | **0.534** | 0.802 | 1.073 | 1.416 |
...
|---|---|---:|---:|---:|---:|---:|
| edge_bucket | `10_to_20pct` | 902 | 1.104 | 0.812 | 0.121 | 2.500 |
| edge_bucket | `5_to_10pct` | 393 | 1.049 | 0.822 | 0.033 | 2.536 |
| edge_bucket | `ge_20pct` | 468 | 1.216 | 1.006 | 0.332 | 2.497 |
| injury_context | `fresh` | 368 | 1.072 | 0.807 | 0.085 | 2.575 |
| injury_context | `latest_valid_report_selected` | 89 | 0.890 | 0.850 | -0.091 | 2.357 |
| injury_context | `unavailable` | 1310 | 1.130 | 0.860 | 0.173 | 2.506 |
| line_bucket | `10_to_15` | 120 | 1.066 | 1.012 | 0.142 | 3.271 |
| line_bucket | `15_to_20` | 120 | 1.178 | 0.930 | 0.416 | 3.871 |
| line_bucket | `3_to_5` | 124 | 0.997 | 0.844 | -0.008 | 2.403 |
| line_bucket | `4_to_7` | 179 | 1.190 | 1.063 | 0.339 | 2.869 |
| line_bucket | `7_to_10` | 92 | 1.155 | 1.049 | 0.341 | 3.067 |
| line_bucket | `lt_10` | 35 | 1.084 | 0.931 | 0.103 | 3.616 |
| line_bucket | `lt_3` | 58 | 1.093 | 0.871 | 0.096 | 2.172 |
| line_bucket | `lt_4` | 164 | 1.099 | 1.049 | 0.130 | 2.640 |
| lineup_confirmed | `projected` | 490 | 1.066 | 0.808 | 0.081 | 2.530 |
| lineup_confirmed | `unavailable` | 1310 | 1.130 | 0.860 | 0.173 | 2.506 |
| low_line_discrete | `no` | 1230 | 1.106 | 0.853 | 0.194 | 2.973 |
| minutes_volatility_bucket | `unavailable` | 1825 | 1.103 | 0.840 | 0.144 | 2.512 |
| overall | `ALL` | 1825 | 1.103 | 0.840 | 0.144 | 2.512 |
| p0_bucket | `5_to_20pct` | 516 | 1.179 | 0.955 | 0.195 | 2.305 |
| p0_bucket | `lt_5pct` | 700 | 1.086 | 0.837 | 0.166 | 3.490 |
| predicted_variance_bucket | `high` | 602 | 1.105 | 0.819 | 0.223 | 3.315 |
| predicted_variance_bucket | `low` | 602 | 1.078 | 1.035 | 0.071 | 1.986 |
| predicted_variance_bucket | `mid` | 621 | 1.107 | 0.931 | 0.137 | 2.243 |
| role_bucket | `lt30min` | 575 | 1.225 | 1.075 | 0.280 | 2.654 |
| role_bucket | `rotation` | 133 | 1.137 | 0.939 | 0.175 | 2.539 |
| side | `OVER` | 251 | 0.907 | 1.007 | -0.264 | 3.033 |
| side | `UNDER` | 1574 | 1.139 | 0.821 | 0.209 | 2.429 |
| snapshot_type | `morning` | 1825 | 1.103 | 0.840 | 0.144 | 2.512 |
| stat | `ast` | 287 | 1.057 | 0.807 | 0.066 | 2.436 |
| stat | `pts` | 385 | 1.100 | 0.827 | 0.222 | 3.693 |
| stat | `reb` | 469 | 1.148 | 0.999 | 0.256 | 2.815 |
| vacated_opportunity_bucket | `unavailable` | 1825 | 1.103 | 0.840 | 0.144 | 2.512 |
...
- settled window: **2026-04-17 → 2026-05-28** (28 dates).
