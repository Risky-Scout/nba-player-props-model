# PMF Variance Experience Study — July 20, 2026

_Actual-to-expected (A/E) review of settled PMF predictions, as-of `2026-07-20` over a 60-day lookback._

## Executive summary

- **391** settled rows from **2026-05-21** through **2026-06-13** (12 delivery dates with at least one settled row).
- **Mean A/E = 1.057** — actual outcomes ran +5.7% relative to expected means in this sample.
- **Variance A/E = 0.793** — PMF spread is reasonably close overall (the well-calibrated band is 0.80–1.20), slightly wide overall.
- **Standardized residual: mean = 0.109, sd = 0.985** — slight positive bias and dispersion close to calibrated (target sd = 1.00).
- Quantile coverage at the 75th and 90th percentiles is near target (0.788 and 0.936); the 10th-percentile band is over-covered (0.199 vs target 0.10).
- **Model trails market on binary scoring:** Brier 0.261 vs 0.247 (model vs market); logloss 0.722 vs 0.690.
...
|---|---:|
| rows | 391 |
| actual_mean (per row) | 5.803 |
| expected_mean (per row) | 5.489 |
| **mean_AE** | **1.0572** |
| Σ squared residual | 4933.03 |
| Σ expected variance | 6217.85 |
| **variance_AE** | **0.7934** |
| standardized_residual_mean | 0.1086 |
| standardized_residual_sd | 0.9854 |
| pmf_nll_mean | 2.2476 |
| pmf_rps_mean | 0.1158 |
| model_brier (over/under) | 0.2613 |
| market_brier (over/under) | 0.2475 |
| model_logloss (over/under) | 0.7224 |
| market_logloss (over/under) | 0.6897 |
| coverage @ 10 / 25 / 50 / 75 / 90 | 0.199 / 0.317 / 0.547 / 0.788 / 0.936 |
...
| Dimension | Bucket | n | variance_AE | std_resid_sd | mean_AE | NLL |
|---|---|---:|---:|---:|---:|---:|
| edge_bucket | `10_to_20pct` | 184 | **0.733** | 1.005 | 1.055 | 2.322 |
...
| low_line_discrete | `no` | 263 | **0.786** | 0.914 | 1.056 | 2.562 |
| minutes_volatility_bucket | `unavailable` | 391 | **0.793** | 0.985 | 1.057 | 2.248 |
| overall | `ALL` | 391 | **0.793** | 0.985 | 1.057 | 2.248 |
| p0_bucket | `lt_5pct` | 139 | **0.704** | 0.839 | 1.035 | 2.913 |
| predicted_variance_bucket | `high` | 129 | **0.774** | 0.904 | 1.058 | 3.060 |
| role_bucket | `ge30min_starter` | 64 | **0.527** | 0.876 | 1.078 | 2.233 |
| role_bucket | `starter` | 158 | **0.788** | 1.018 | 1.062 | 2.476 |
| side | `UNDER` | 321 | **0.776** | 0.953 | 1.111 | 2.267 |
| snapshot_type | `morning` | 391 | **0.793** | 0.985 | 1.057 | 2.248 |
| stat | `ast` | 63 | **0.559** | 0.767 | 1.028 | 2.040 |
| stat | `pts` | 84 | **0.786** | 0.936 | 1.063 | 3.319 |
| vacated_opportunity_bucket | `unavailable` | 391 | **0.793** | 0.985 | 1.057 | 2.248 |
...
| predicted_variance_bucket | `mid` | 133 | 1.051 | 0.856 | 0.098 | 2.071 |
...
| line_bucket | `15_to_20` | 18 | 0.905 | -0.009 |
...
- settled window: **2026-05-21 → 2026-06-13** (12 dates).
