# PMF Variance Experience Study — July 24, 2026

_Actual-to-expected (A/E) review of settled PMF predictions, as-of `2026-07-24` over a 60-day lookback._

## Executive summary

- **295** settled rows from **2026-05-25** through **2026-06-13** (8 delivery dates with at least one settled row).
- **Mean A/E = 1.074** — actual outcomes ran +7.4% relative to expected means in this sample.
- **Variance A/E = 0.815** — PMF spread is reasonably close overall (the well-calibrated band is 0.80–1.20), slightly wide overall.
- **Standardized residual: mean = 0.149, sd = 1.020** — slight positive bias and dispersion close to calibrated (target sd = 1.00).
- Quantile coverage at the 75th and 90th percentiles is near target (0.776 and 0.929); the 10th-percentile band is over-covered (0.210 vs target 0.10).
- **Model trails market on binary scoring:** Brier 0.272 vs 0.249 (model vs market); logloss 0.747 vs 0.694.
...
|---|---:|
| rows | 295 |
| actual_mean (per row) | 5.597 |
| expected_mean (per row) | 5.213 |
| **mean_AE** | **1.0735** |
| Σ squared residual | 3534.66 |
| Σ expected variance | 4334.61 |
| **variance_AE** | **0.8155** |
| standardized_residual_mean | 0.1488 |
| standardized_residual_sd | 1.0200 |
| pmf_nll_mean | 2.2550 |
| pmf_rps_mean | 0.1231 |
| model_brier (over/under) | 0.2724 |
| market_brier (over/under) | 0.2494 |
| model_logloss (over/under) | 0.7473 |
| market_logloss (over/under) | 0.6942 |
| coverage @ 10 / 25 / 50 / 75 / 90 | 0.210 / 0.312 / 0.529 / 0.776 / 0.929 |
...
|---|---|---:|---:|---:|---:|---:|
| line_bucket | `1_to_1p5` | 40 | **1.533** | 1.291 | 1.374 | 2.155 |
| line_bucket | `le_half` | 63 | **1.215** | 1.057 | 0.966 | 1.365 |
| low_line_discrete | `yes` | 103 | **1.367** | 1.167 | 1.172 | 1.672 |
| p0_bucket | `20_to_50pct` | 66 | **1.373** | 1.290 | 1.103 | 2.053 |
| predicted_variance_bucket | `low` | 98 | **1.387** | 1.171 | 1.134 | 1.641 |
| role_bucket | `core` | 38 | **1.268** | 0.991 | 0.993 | 2.307 |
| side | `OVER` | 53 | **1.215** | 0.985 | 0.724 | 2.124 |
| stat | `fg3m` | 52 | **1.415** | 1.294 | 1.016 | 1.935 |
...
| Dimension | Bucket | n | variance_AE | std_resid_sd | mean_AE | NLL |
|---|---|---:|---:|---:|---:|---:|
| edge_bucket | `10_to_20pct` | 133 | **0.679** | 1.054 | 1.066 | 2.357 |
| injury_context | `unavailable` | 62 | **0.574** | 0.912 | 1.099 | 2.170 |
| lineup_confirmed | `unavailable` | 62 | **0.574** | 0.912 | 1.099 | 2.170 |
| p0_bucket | `lt_5pct` | 102 | **0.753** | 0.856 | 1.051 | 2.906 |
| predicted_variance_bucket | `high` | 98 | **0.784** | 0.902 | 1.063 | 3.005 |
| role_bucket | `ge30min_starter` | 42 | **0.591** | 0.931 | 1.135 | 2.283 |
| role_bucket | `rotation` | 40 | **0.799** | 1.142 | 1.268 | 2.102 |
| side | `UNDER` | 242 | **0.766** | 0.980 | 1.128 | 2.284 |
| stat | `ast` | 48 | **0.579** | 0.813 | 1.005 | 2.059 |
| stat | `pts` | 59 | **0.795** | 0.915 | 1.067 | 3.312 |
...
|---|---|---:|---:|---:|---:|---:|
| edge_bucket | `5_to_10pct` | 95 | 1.049 | 1.089 | 0.160 | 2.214 |
...
| low_line_discrete | `no` | 192 | 1.067 | 0.803 | 0.129 | 2.568 |
| minutes_volatility_bucket | `unavailable` | 295 | 1.074 | 0.815 | 0.149 | 2.255 |
| overall | `ALL` | 295 | 1.074 | 0.815 | 0.149 | 2.255 |
| p0_bucket | `5_to_20pct` | 86 | 1.122 | 1.173 | 0.165 | 2.166 |
| p0_bucket | `ge_50pct` | 41 | 1.604 | 1.012 | 0.335 | 1.148 |
| predicted_variance_bucket | `mid` | 99 | 1.096 | 0.970 | 0.170 | 2.121 |
| role_bucket | `starter` | 129 | 1.057 | 0.810 | 0.185 | 2.475 |
| snapshot_type | `morning` | 295 | 1.074 | 0.815 | 0.149 | 2.255 |
| stat | `blk` | 31 | 1.445 | 1.109 | 0.399 | 1.297 |
| stat | `reb` | 67 | 1.090 | 0.923 | 0.174 | 2.501 |
| stat | `stl` | 38 | 1.326 | 1.176 | 0.230 | 1.648 |
| vacated_opportunity_bucket | `unavailable` | 295 | 1.074 | 0.815 | 0.149 | 2.255 |
...
|---|---|---:|---:|---:|
| edge_bucket | `0_to_5pct` | 23 | 0.609 | -0.164 |
| line_bucket | `10_to_15` | 25 | 0.816 | 0.038 |
| line_bucket | `15_to_20` | 13 | 1.024 | -0.012 |
| line_bucket | `2_to_2p5` | 14 | 0.888 | 0.157 |
| line_bucket | `3_to_5` | 22 | 0.562 | 0.005 |
| line_bucket | `4_to_7` | 21 | 1.321 | 0.231 |
| line_bucket | `5_to_8` | 14 | 0.495 | -0.066 |
| line_bucket | `7_to_10` | 10 | 1.061 | 0.253 |
| line_bucket | `ge_10` | 12 | 0.474 | 0.236 |
| line_bucket | `ge_25` | 13 | 0.624 | 0.471 |
| line_bucket | `ge_3` | 4 | 1.507 | 0.488 |
| line_bucket | `lt_10` | 8 | 0.876 | 0.026 |
...
- settled window: **2026-05-25 → 2026-06-13** (8 dates).
