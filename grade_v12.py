#!/usr/bin/env python3
"""
grade_v12.py — NBA Player Props Model Grading Engine
VERSION: 2026-03-09-v12

Syndicate-grade grading: CLV tracking, ROI, calibration, drawdown analysis.

Reads:  predictions/singles_{date}.json
Writes: graded/graded_{date}.csv
        graded/performance_log.csv
        graded/cumulative_report.json

Primary metric: CLV (Closing Line Value)
  Beating the closing line consistently is the most reliable indicator
  that the model is finding real inefficiencies, not overfitting.
"""
...
def grade_single(pred: dict, actual_stat) -> dict:
    ...
    # CLV: how much model probability exceeded the no-vig market implied prob
    if side == "OVER":
...
def fetch_actual_stats(target_date: str) -> dict:
    ...
    for rec in records:
        ...
        if not pid:
            continue
        from bdl_client import parse_minutes
    ...
    logger.info(
        f"Fetched actuals: {len(records)} stat records → "
        f"{len(result)} (player, stat) pairs"
    )
    return result
...
def grade_date(target_date: str) -> pd.DataFrame:
    ...
    # Handle both dict format {"picks": [...]} and legacy list format
    ...
    if not actuals:
        logger.warning("No actuals fetched — all picks marked NO_ACTION.")
...
def print_report(df: pd.DataFrame, date_label: str = ""):
    ...
    print(f"\n{'='*72}")
    print(f"NBA Player Props Model  GRADING REPORT — {date_label}")
    print(f"{'='*72}")
    print(
        f"  Predictions: {len(df):4d}  |  Graded: {len(active):4d}  |  "
        f"NO_ACTION: {n_no_action}  |  PUSH: {n_push}"
    )
    print(f"  {'-'*68}")
    ...
    print(
        f"  TOTAL    Bets: {total_m['n_bets']:3d}  Hit: {total_m['hit_rate']:.1%}  "
    ...
    for stat in STATS:
        ...
        print(
            f"    {STAT_DISPLAY[stat]:10s} Bets: {m['n_bets']:3d}  "
            f"Hit: {m['hit_rate']:.1%}  ROI: {m['roi']:+.1%}  CLV: {m['mean_clv']:+.4f}"
        )
    ...
    if "model_prob" in bet.columns:
        ...
        for lo, hi in [
            (0.40,0.50),(0.50,0.55),(0.55,0.60),
            (0.60,0.65),(0.65,0.70),(0.70,1.0)
        ]:
    ...
    if len(clv_vals) > 0:
        ...
        print(
            f"    Mean: {clv_vals.mean():+.4f}  Std: {clv_vals.std():.4f}  "
            f"Positive: {(clv_vals > 0).mean():.1%}  "
            f"P10: {np.percentile(clv_vals,10):+.3f}  "
            f"P90: {np.percentile(clv_vals,90):+.3f}"
        )

    print(
        f"  {'-'*68}\n"
        f"  Brier: {total_m.get('brier','N/A')}  |  "
        f"Sharpe: {total_m['sharpe']:.3f}  |  "
        f"Max Drawdown: {total_m['max_dd']:.2f}u\n"
    )
...
def update_performance_log(df: pd.DataFrame, target_date: str):
    ...
    if PERF_LOG.exists():
        ...
        if "grade_date" in existing.columns:
            existing = existing[existing["grade_date"] != target_date]
    ...
    if "grade_date" in bet.columns:
        ...
        if len(all_dates) >= 1:
            for window, key in [(30, "rolling_30"), (60, "rolling_60")]:
                cutoff = (
                    datetime.strptime(all_dates[-1], "%Y-%m-%d") - timedelta(days=window)
                ).strftime("%Y-%m-%d")
...
def print_cumulative_report():
    ...
    print(f"\n{'='*72}")
    print(f"NBA Player Props Model  CUMULATIVE PERFORMANCE REPORT")
    ...
    def show_metrics(m):
        if not m or m.get("n_bets", 0) == 0:
            ...
            return
        print(
            f"    Bets: {m['n_bets']:4d}  Hit: {m['hit_rate']:.1%}  "
            f"ROI: {m['roi']:+.1%}  P&L: {m['profit']:+.2f}  "
            f"CLV: {m['mean_clv']:+.4f}  MaxDD: {m['max_dd']:.2f}"
        )
    ...
    for stat in STATS:
        ...
        if m.get("n_bets", 0) > 0:
            print(f"  {STAT_DISPLAY[stat]:10s}", end="")
            show_metrics(m)
    ...
    if "model_prob" in bet.columns:
        ...
        for lo, hi in [
            ...
            act_p  = float((bet[mask]["result"] == "HIT").mean())
            print(
                f"  {lo:.0%}-{hi:.0%}      {mask.sum():5d}  "
                f"{pred_p:7.3f}  {act_p:7.3f}  {abs(pred_p-act_p):7.3f}"
            )

    if len(bet) > 0:
        pnl_cum = (
            bet.sort_values("grade_date")["profit"].cumsum().values
            if "grade_date" in bet.columns
            else bet["profit"].cumsum().values
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="NBA Player Props Model — Grading Engine"
    )
    parser.add_argument("--date",   type=str, default=None,
                        help="Date to grade (YYYY-MM-DD). Defaults to yesterday.")
    parser.add_argument("--report", action="store_true",
                        help="Print cumulative performance report and exit.")
    ...
    logger.info("=" * 72)
    logger.info(f"NBA Player Props Model  GRADING — {target}")
    logger.info("VERSION: 2026-03-09-v12")
    logger.info("=" * 72)
