...
def test_no_games_helper_true_when_manifest_says_so(tmp_path, monkeypatch):
    mod = _load_publish_module(monkeypatch)
    monkeypatch.chdir(tmp_path)
    _write_no_games_manifest(tmp_path, "2026-05-16")
    assert mod._m8_6o_delivery_manifest_no_games_slate("2026-05-16") is True


def test_no_games_helper_false_when_manifest_missing(tmp_path, monkeypatch):
    mod = _load_publish_module(monkeypatch)
    monkeypatch.chdir(tmp_path)
    assert mod._m8_6o_delivery_manifest_no_games_slate("2026-05-16") is False


def test_no_games_helper_false_when_flag_false(tmp_path, monkeypatch):
    mod = _load_publish_module(monkeypatch)
    monkeypatch.chdir(tmp_path)
...
def test_no_games_helper_false_when_reason_differs(tmp_path, monkeypatch):
    """Even if no_games_slate=true, the reason field must say
...
def test_no_games_helper_false_when_date_none(tmp_path, monkeypatch):
    mod = _load_publish_module(monkeypatch)
    monkeypatch.chdir(tmp_path)
    assert mod._m8_6o_delivery_manifest_no_games_slate(None) is False


def test_pmf_research_builder_hook_soft_skips_on_no_games_slate(
    ...
    that builder hard-fails on a row-empty PMF parquet."""
    mod = _load_publish_module(monkeypatch)
    monkeypatch.chdir(tmp_path)
    _write_no_games_manifest(tmp_path, "2026-05-16")
    # Make sure the builder path the hook checks exists, otherwise the
    # hook raises SystemExit before reaching our no-games gate.
...
def test_pmf_research_builder_hook_runs_normally_when_no_signal(
    tmp_path, monkeypatch, capsys
):
    """Without the no-games flag, the hook must subprocess into the
    real builder as before (no silent skip on a games-bearing slate)."""
    mod = _load_publish_module(monkeypatch)
    monkeypatch.chdir(tmp_path)
    (tmp_path / "scripts").mkdir(exist_ok=True)
