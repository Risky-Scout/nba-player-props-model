...
- model: `d1ddc807#phase10c`

### Caveats

Full detail (including the `required_to_resolve` field for each blocker) is in `wizard_of_odds/run_manifest.json`.

- `injury_very_stale` — No canonical delivery row had a row-level injury freshness verdict of fresh. row_level_injury_fresh_rows=0/180; dominant_status='unknown' (count=180); sample_injury_report_fetched_at_utc='2026-05-23T20:46:57Z'; reason=injury_report_not_yet_published_or_unavailable
...
- **tov_status**: `present`
- **row counts**: fair_odds_board=3855, full_pmfs_wide=180, market_comparison=2474, publishable_edges=1666
- **fair_odds_board diagnostics**: `{'fair_over_odds_null_count': 568, 'fair_under_odds_null_count': 568, 'zero_or_one_prob_count': 300}`
